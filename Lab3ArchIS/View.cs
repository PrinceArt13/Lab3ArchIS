    ...
    class View
    {
        ...
        static void doClientRequest(ref Connection con)
        {
            switch (con.Act)
            {
                ...
                #endregion
                #region case DB
                case "DBGetAllRecords":
                    List<string> allrec = DBController.GetInstance().GetAllRecords();
                    con.Content = allrec;
                    logger.Info("Client received all records");
                    break;
                case "DBGetSepRecord":
                    try
                    {
                        List<string> rec = DBController.GetInstance().GetSepRecord(int.Parse(con.Content[0]));
                        con.Content = rec;
                        logger.Info("Client received separate record");
                    }
                    catch (Exception ex)
                ...
                case "DBDeleteRecord":
                    try
                    {
                        DBController.GetInstance().DeleteRecord(int.Parse(con.Content[0]));
                        con.Content = new List<string> { "True" };
                        logger.Info("Client succesfully deleted the record");
                    }
                    catch (Exception ex)
