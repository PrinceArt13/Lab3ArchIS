...
namespace ClientLab2
{
    class ClientController
    {
        ...
        #region DBhandler
        public void DBAddRecord(string newRecord)
        {
            SendRequest(new Connection("DBAddRecord", newRecord));
        }

        public bool DBDeleteRecord(int number)
        {
            Connection con = SendRequest(new Connection("DBDeleteRecord", number.ToString()));
            return (con.Content[0] == "True");
        }

        public List<string> DBGetAllRecords()
        {
            Connection con = SendRequest(new Connection("DBGetAllRecords", ""));
            return con.Content;
        }

        public List<string> DBGetSepRecord(int number)
        {
            Connection con = SendRequest(new Connection("DBGetSepRecord", number.ToString()));
            return con.Content;
        }
        #endregion
    }
